...
//returns 1 if in configuration mode
int get_config_mode();
...
typedef struct melty_parameters_t {
  ...
  float throttle_percent;  //stores throttle percent
	...
	unsigned long motor_start_phase_1; //time offset for when motor 1 turns on when translating forward
	unsigned long motor_stop_phase_1;  //time offset for when motor 1 turns off when translating forward
	unsigned long motor_start_phase_2; //"" for motor 2 (times can overlap with motor 1)
	unsigned long motor_stop_phase_2;  //""
