...
typedef struct melty_parameters_t {
  ...
  float throttle_percent;  //stores throttle percent
	...
	unsigned long motor_start1; //time offset for when motor 1 turns on (when translating forward)
	unsigned long motor_stop1;  //time offset for when motor 1 turns off (when translating forward)
